...
const MessageItem = memo(({ text, role }) => {
  ...
  const wrapperClass = isBot
    ? 'flex items-start gap-2 md:gap-3 mb-6 md:mb-8'
    : 'flex items-start gap-2 md:gap-3 flex-row-reverse mb-6 md:mb-8';
  ...
  return (
    <div className={wrapperClass}>
      {isBot ? (
        ...
        <div
          className={`flex flex-shrink-0 justify-center items-center w-7 h-7 rounded-full md:w-8 md:h-8 ${userBubbleClass}`}
        >
      ...
      <div
        className={`p-3 md:p-4 rounded-lg max-w-[85%] md:max-w-2xl text-white ${
